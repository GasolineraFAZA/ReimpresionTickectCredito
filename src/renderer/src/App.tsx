...
/** Convierte el logo importado a data URI base64 (para inyectarlo en la impresión). */
async function getLogoBase64(): Promise<string> {
  const resp = await fetch(logoPunto)
...
export default function App(): React.JSX.Element {
  ...
  // Datos de la estación (solo lo que viene de la API) para el encabezado del ticket
  const estacionInfo = sucursal
    ? { clave: sucursal.referencia4, nombre: sucursal.nombre, direccion: sucursal.direccion }
    : undefined
  ...
  const handleImprimir = async () => {
    ...
    try {
      await verificarYContinuar(async (usuario) => {
        ...
        setMensaje('Enviando a impresora...')

        // Generar el HTML del ticket con el logo en base64
        const logoBase64 = await getLogoBase64()
        const html = renderToStaticMarkup(
          <TicketPreview
            folio={resultado.folio}
            pagoTarjeta={resultado.pagoTarjeta}
            d={resultado.despacho}
            estacion={estacionInfo}
            logoSrc={logoBase64}
          />
        )

        await window.electron.imprimirTicketTermico(html, impresora, Number(cantidad) || 1)
        await registrarReimpresion(resultado.despacho.gasolinera, usuario)

        setMensaje(`Ticket enviado a ${impresora} (${cantidad} copia${Number(cantidad) > 1 ? 's' : ''})`)
      })
    } catch (e: any) {
      setError(`Error al imprimir: ${e?.message ?? String(e)}`)
    } finally {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
          ...
          <div className="bg-white rounded-xl shadow-2xl flex flex-col max-h-[90vh] w-72 overflow-hidden"
            ...
            <div className="overflow-y-auto flex-1 p-3 bg-gray-50">
              <TicketPreview folio={preview.folio} pagoTarjeta={preview.pagoTarjeta} d={preview.despacho} estacion={estacionInfo} />
            </div>
